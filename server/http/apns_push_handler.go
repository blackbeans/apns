...
func (self *ApnsHttpServer) decodePayload(req *http.Request, resp *response) (string, *entry.PayLoad) {
	...
	if !noToken {
		resp.Status = server.RESP_STATUS_PUSH_ARGUMENTS_INVALID
	...
	if len(extArgs) > 0 {
		...
		if nil != err {
			resp.Status = server.RESP_STATUS_PAYLOAD_BODY_DECODE_ERROR
			resp.Error = errors.New("PAYLOAD BODY DECODE ERROR!")
		} else {
			for k, v := range jsonMap {
				//如果存在数据嵌套则返回错误，不允许数据多层嵌套
				if reflect.TypeOf(v).Kind() == reflect.Map {
					resp.Status = server.RESP_STATUS_PAYLOAD_BODY_DEEP_ITERATOR
...
func (self *ApnsHttpServer) innerSend(pushType string, token string, payload *entry.PayLoad, resp *response, expiredTime uint32) {
	...
	if server.NOTIFY_SIMPLE_FORMAT == pushType {
		//如果为简单
		sendFunc = func() error {
			return self.apnsClient.SendSimpleNotification(token, *payload)
		}
	} else if server.NOTIFY_ENHANCED_FORMAT == pushType {
		//如果为扩展的
		sendFunc = func() error {
		...
		resp.Status = server.RESP_STATUS_INVALID_NOTIFY_FORMAT
	...
	if nil != err {
		...
		resp.Status = server.RESP_STATUS_SEND_OVER_TRY_ERROR
		resp.Error = err
	}
}
