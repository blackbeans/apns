...
type ApnsClient struct {
	...
	sendCounter     *Counter
	failCounter     *Counter
	resendCounter   *Counter
}

func NewDefaultApnsClient(cert tls.Certificate, pushGateway string,
	feedbackChan chan<- *Feedback, feedbackGateWay string,
	storage IMessageStorage) *ApnsClient {

	//发送失败后的响应channel
	respChan := make(chan *Response, 1000)

	deadline := 10 * time.Second
	err, factory := NewConnPool(5, 5, 5, 10*time.Minute, func(id int32) (error, IConn) {
...
func NewApnsClient(factory IConnFactory, feedbackFactory IConnFactory, storage IMessageStorage) *ApnsClient {
	//发送失败后的响应channel
	respChan := make(chan *Response, 1000)
	return newApnsClient(factory, feedbackFactory, storage, respChan)
}
...
func newApnsClient(factory IConnFactory, feedbackFactory IConnFactory,
	storage IMessageStorage, responseChannel chan *Response) *ApnsClient {

	client := &ApnsClient{factory: factory, feedbackFactory: feedbackFactory,
		running: true, maxttl: 3, storage: storage, sendCounter: &Counter{}, failCounter: &Counter{}, resendCounter: &Counter{}}
	go func() {
		for client.running {
...
//发送简单的notification
func (self *ApnsClient) SendSimpleNotification(deviceToken string, payload PayLoad) error {
	message := NewMessage(CMD_SIMPLE_NOTIFY, self.maxttl, MESSAGE_TYPE_SIMPLE)
	token, err := WrapDeviceToken(deviceToken)
	if nil != err {
		return err
	}
	pl, err := WrapPayLoad(&payload)
	if nil != err {
		return err
	}
...
func (self *ApnsClient) SendEnhancedNotification(expiriedTime uint32, deviceToken string, pl PayLoad) error {

	message := NewMessage(CMD_ENHANCE_NOTIFY, self.maxttl, MESSAGE_TYPE_ENHANCED)
	payload, err := WrapPayLoad(&pl)
	...
	message.AddItem(WrapExpirationDate(expiry), token, payload)

	return self.sendMessage(message)
}

func (self *ApnsClient) sendMessage(msg *Message) error {
	var sendError error
	//重发逻辑
	for i := 0; i < 3; i++ {
